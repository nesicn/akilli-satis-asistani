...
# Dinamik XAI (Explainable AI) Karar Gerekçesi Oluşturucu
def generate_xai_insights(secili_musteri, harcanan_tutar, secilen_kategori, proba):
    ...
    if secili_musteri:
        ...
        if gecen_gun_col in secili_musteri and ort_aralik_col in secili_musteri:
            gecen_gun = float(secili_musteri.get(gecen_gun_col, 0))
            ort_aralik = float(secili_musteri.get(ort_aralik_col, 1))
            if gecen_gun >= ort_aralik and ort_aralik > 0:
        ...
        if tuketim_orani_col in secili_musteri:
            tuketim_orani = float(secili_musteri.get(tuketim_orani_col, 0))
    ...
    if secili_musteri and 'gecmis_islem_sayisi' in secili_musteri:
        islem_sayisi = float(secili_musteri.get('gecmis_islem_sayisi', 0))
        if islem_sayisi > 15:
            insights.append(f"🔄 **Sık Alışveriş Yapan Müşteri:** Geçmişteki {int(islem_sayisi)} başarılı işlem güven indeksini yükseltiyor.")
...
with tab_kasa:
    ...
    with col_left:
        ...
        secili_musteri = None
        if telefon_input and not df_musteriler.empty:
    ...
    with col_right:
        ...
        if st.session_state.analiz_yapildi:
            if model is None:
                ...
                # --- A. DİNAMİK FEATURE VEKTÖRÜ OLUŞTURMA ---
                input_dict = {
                    "Harcanan_Tutar_TL": harcanan_tutar,
                ...
                try:
                    ...
                    st.warning(f"Model tahmini sırasında bir uyumsuzluk saptandı: {str(e)}.")
                    proba = 0.50

                st.divider()

                # --- C. AÇIKLANABİLİR YAPAY ZEKA (XAI) ---
                ...
                # --- E. CANLI GERİ BİLDİRİM DÖNGÜSÜ (FEEDBACK LOOP) ---
...
with tab_analitik:
    ...
    if not df_musteriler.empty:
        ...
        col_m2.metric("VIP Müşteri Sayısı", len(df_musteriler[df_musteriler['Segment'].astype(str).str.contains('VIP')]))
        
        ort_sepet = df_musteriler['gecmis_ortalama_sepet_tutari'].mean() if 'gecmis_ortalama_sepet_tutari' in df_musteriler.columns else 0.0
        col_m3.metric("Ort. Harcama Tutarı", f"{ort_sepet:.1f} TL" if ort_sepet > 0 else "N/A")
        
        ort_kidem = df_musteriler['musteri_kidem_gun'].mean() if 'musteri_kidem_gun' in df_musteriler.columns else 0.0
        col_m4.metric("Ort. Müşteri Kıdemi", f"{ort_kidem:.0f} Gün" if ort_kidem > 0 else "N/A")
